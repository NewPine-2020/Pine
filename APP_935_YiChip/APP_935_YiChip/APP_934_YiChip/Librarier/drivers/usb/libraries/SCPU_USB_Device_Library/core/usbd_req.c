...
//        HWRITE_INLINE(&pdev->regs.INDEXREGS->CSRL.CSR0L, csr0l.d8);
...
//    csr0l.d8 = USBHREAD(&pdev->regs.INDEXREGS->CSRL.CSR0L);
    ...
    case USB_OTG_CONFIGURED:
        ...
        if (HIBYTE(((req->wIndex)&0x7f00) ) <= USBD_ITF_MAX_NUM) 
        {
//            csr0l.b.serviced_rxpktrdy = 1;
//            HWRITE_INLINE(&pdev->regs.INDEXREGS->CSRL.CSR0L, csr0l.d8);
...
//    csr0l.d8 = USB_OTG_READ_REG8(&pdev->regs.INDEXREGS->CSRL.CSR0L);
	  if((req->bmRequest == 0xA2) |(req->bmRequest == 0x22))
  	{
  		pdev->dev.class_cb->Setup (pdev, req);
   	 }
...
//		DEBUG(("repond USB_DESC_TYPE_STRING (uint8_t)(req->wValue) = %x\r\n",(uint8_t)(req->wValue)));
...
//      USBD_CtlSendStatus(pdev); 
      USBHWRITE(&pdev->regs.CTRLREGS->FADDR,Address_Value);
      USBD_CtlSendStatus(pdev);
...
//           ((req->wIndex & 0xFF) == 0))
//    {
 //       dctl.d8 = USBHREAD(&pdev->regs.COMMREGS->TESTMODE);
//        dctl.d8 = 0;
                            ...
                            USB_SETUP_REQ *req)
{
    ...
    {
    	//MyPrintf("req ->bmRequest =%x\n",*(uint8_t *)  (pdev->dev.setup_packet+i));
    }
}
