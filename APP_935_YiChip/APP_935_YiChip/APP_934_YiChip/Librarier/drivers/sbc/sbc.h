...
typedef struct tagSBCData  
{
  ...
  unsigned short allocation_method;
  unsigned short nrof_subbands;
  unsigned short nrof_blocks;
  unsigned short nrof_channels;
  unsigned short sampling_frequency; /* 0..3 */
  int bitpool;
  unsigned char join;
} TSBCData;
//typedef struct tagSBCData  
...
int sbcDecodeGetFrameSize(unsigned char* pInputBuffer);
#endif
