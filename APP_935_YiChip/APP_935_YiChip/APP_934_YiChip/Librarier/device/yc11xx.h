...
#define CORE_ADCD_DCOFF1				 0X813b
#define CORE_ADCD_DELAY1				 0X813d
#define CORE_ADCD_CTRL1				 0X813e
#define CORE_ADCD_SADDR1				 0X813f
#define CORE_ADCD_EADDR1				 0X8141
#define CORE_ADCD_DCOFF2				 0X8143
#define CORE_ADCD_DELAY2				 0X8145
#define CORE_ADCD_CTRL2				 0X8146
#define CORE_ADCD_SADDR2				 0X8147
#define CORE_ADCD_EADDR2				 0X8149

#define CORE_DAC_ANC_CTRL       				 0X814b
#define CORE_DAC_AMP_LIMIT				 0X814c
#define CORE_ANC_CTRL				 0X814e
...
#define CORE_GPIO_WAKEUP_HIGH                0x8347
...
#define CORE_LPM_READ_REG		            0X8380

#define CORE_ADCD_ADDR1                       0x8384
#define CORE_ADCD_ADDR2                       0x8386
...
#define HREAD_INLINE(reg)			((HREADDEF(reg)))
#define HREADW_INLINE(reg)			((int)HREAD_INLINE(reg) | HREAD_INLINE(reg + 1) << 8)

#define HWRITE_INLINE(reg,val)			do {(HREADDEF(reg) = (byte)(val));}while(0)
...
#define USB_OTG_MODIFY_REG8(reg, clear_mask, set_mask)  \
        USBHWRITE(reg, (((USBHREAD(reg)) & ~clear_mask) | set_mask))
...
/***********************************core_clkoff2*************************************************/
#define REG_CLOCK_OFF2_EQ                	(0x0001)
#define REG_CLOCK_OFF2_ANC                		(0x0002)
...
#define LPMREG_SEL_ADC1                          0x40
#define LPMREG_SEL_ADC2                          0x80


typedef struct     
{     
    unsigned char charge_ctrl:1;//(4'b0,lpm_charger_event_en,lpm_charger_ctrl[26:0])
    unsigned char xo32k:1;//(7'b0,lpm_xo32k_sel,lpm_xo32k_ctrl[23:0])
    unsigned char chgr_wk:1;//(7'b0,chgr_ctrl[4:0],chgr_event_wkhi,chgr_event_wklow)
    unsigned char voice_det_ctrl:1;//(5'b0,voice_det_ctrl)
    unsigned char adc_ctrl:1;//(lpm_adc_ctrl[31:0])
    unsigned char adc_ctrl_high:1;//(16'b0,misc_gbg_ctrl[7:0],2'b0,lpm_adc_ctrl[37:32])//error
    unsigned char adc1_ctrl:1;//(1'b0,lpm_adc1_ctrl[30:0])
    unsigned char adc2_ctrl:1;//(2'b0,lpm_adc2_ctrl[29:0])
}Lpm_RdMuxRegDef;//CORE_LPM_RD_MUX


typedef struct     
{     
    unsigned int bit_rg_aadc_clksel:2;
    unsigned int bit_rg_aadc_ibc_refbuf2:1;
    unsigned int bit_rg_aadc_ibc_refbuf:3;
    unsigned int bit_rg_aadc_refbuf_vref_ctrl:2;
    unsigned int bit_rg_aadc_rega_vctrl:2;
    unsigned int bit_rg_aadc_regd_vctrl:2;
    unsigned int bit_rg_aadc_vctrl_biasgen:3;
    unsigned int bit_rg_mic_bias_vc:2;
    unsigned int bit_rg_mic_diff_en:1;
    unsigned int bit_rg_mic_op_cm_en:1;
    unsigned int bit_rg_mic_pga_vcmsettle_en:1;
    unsigned int bit_rg_aadc_refbuf_en:1;
    unsigned int bit_da_aadc_en:1;
    unsigned int bit_da_aadc_en_biasgen:1;
    unsigned int bit_da_aadc_en_constgm:1;
    unsigned int bit_da_aadc_en_reg:1;
    unsigned int bit_da_mic_pga_en:1;
    unsigned int bit_da_mic_bias_en:1;
    unsigned int bit_da_mic_pga_gc:3;
    unsigned int bit_da_anc_adc_ctrl:1;//if only enable ff adc, can open imed, if you want open ff and fb, should enbale after double adc en.
}Lpm_Adc1ReadRegDef;//CORE_LPM_READ_REG
